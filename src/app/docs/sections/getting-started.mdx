...
<CodeTabs tabs={[
  { label: "pip", language: "bash", code: "pip install corp-entity-db" },
  { label: "uv", language: "bash", code: "uv add corp-entity-db" },
]} />
...
<CodeTabs tabs={[
  { label: "pip", language: "bash", code: `# Default: search and resolve (no build dependencies)
...
pip install "corp-entity-db[all]"` },
  { label: "uv", language: "bash", code: `# Default: search and resolve (no build dependencies)
uv add corp-entity-db

# With database build/import support (orjson, indexed-bzip2)
uv add "corp-entity-db[build]"

# With HTTP server (corp-entity-db serve)
uv add "corp-entity-db[serve]"

# With remote client (EntityDBClient via httpx)
uv add "corp-entity-db[client]"

# Everything
uv add "corp-entity-db[all]"` },
]} />
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>sqlite-vec</td>
      <td>latest</td>
      <td>Optional (<code>[build]</code> extra), for database construction</td>
    </tr>
    <tr>
      <td>httpx</td>
      <td>latest</td>
      <td>Optional (<code>[client]</code> extra), for EntityDBClient</td>
    </tr>
    <tr>
      <td>FastAPI + uvicorn</td>
      <td>latest</td>
      <td>Optional (<code>[serve]</code> extra), for HTTP server</td>
    </tr>
    <tr>
